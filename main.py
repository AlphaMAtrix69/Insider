...
def main():
    ...
    try:
        original_df, processed_df, check_needed_df, analyzed_df, results_data, excluded_df = process_vulnerability_data(
            vuln_df, kev_set, output_dir, clear_cache
        )
        
        # Generate reports
        report_engine = ReportEngine(output_dir)
        excel_file, json_file, enhanced_excel, *_ = report_engine.generate_all_reports(
            original_df, processed_df, check_needed_df, analyzed_df, results_data, excluded_df
        )
