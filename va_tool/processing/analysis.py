...
def analyze_vulnerability_data(df, kev_set, cve_cache):
    ...
    results = {
        ...
        "eol_duration_summary": generate_eol_duration_summary(analyzed_df)  # Added EOL duration summary
    }
...
def generate_eol_duration_summary(df):
    """
    Generate summary of EOL (End of Life) components by duration.
    
    Args:
        df: DataFrame with vulnerability data
    
    Returns:
        Dictionary with EOL duration summary data
    """
    logger.info("Generating EOL duration summary")
    
    # Filter EOL components
    ...
    if eol_df.empty:
        logger.info("No EOL components found")
        return {
            "total_eol_components": 0,
            "duration_summary": [],
            "eol_details": []
        }
    ...
    # Calculate duration if CVE Published Date is available
    if 'CVE Published Date' in eol_df.columns:
        eol_df['EOL Duration Days'] = eol_df['CVE Published Date'].apply(
            lambda x: (current_date - datetime.datetime.fromisoformat(x).date()).days 
            if pd.notna(x) and x else None
        )
    else:
        # Use Days After Discovery if available
        if 'Days After Discovery' in eol_df.columns:
            eol_df['EOL Duration Days'] = eol_df['Days After Discovery']
        else:
            eol_df['EOL Duration Days'] = None
    ...
    duration_counts = eol_df['Duration Category'].value_counts().reset_index()
    duration_counts.columns = ['Duration', 'Component Count']
    
    # Set priority order for categories
    duration_order = ["< 30 days", "30-90 days", "90-180 days", "180-365 days", "> 365 days", "Unknown"]
    duration_counts['Order'] = duration_counts['Duration'].apply(lambda x: duration_order.index(x))
    duration_counts = duration_counts.sort_values('Order').drop('Order', axis=1)
    
    # Create detailed EOL components list
    eol_details = []
    for _, row in eol_df.iterrows():
        eol_details.append({
            'Plugin ID': str(row.get('Plugin ID', '')),
            'Name': str(row.get('Name', '')),
            'Host': str(row.get('Host', '')),
            'EOL Duration Days': row.get('EOL Duration Days', None),
            'Risk': str(row.get('Risk', '')),
            'CVE': str(row.get('CVE', ''))
        })
    
    # Convert duration_counts to list of dictionaries for JSON
    duration_summary = clean_for_json(duration_counts)
    
    logger.info(f"Found {len(eol_df)} EOL components")
    
    # Return summary data
    return {
        "total_eol_components": len(eol_df),
        "duration_summary": duration_summary,
        "eol_details": eol_details  # This is a list of dictionaries
    }